...
#include "adapter/bignum_mul_bignum_benchmark_adapter.h"

int main(int argc, char **argv)
{
    benchmark_adapter_t adapter;
    bignum_mul_bignum_benchmark_status_t adapter_status;
    benchmark_core_status_t core_status;

    adapter_status = bignum_mul_bignum_benchmark_adapter_init(&adapter);
    if (adapter_status != BIGNUM_MUL_BIGNUM_BENCHMARK_STATUS_SUCCESS) return EXIT_FAILURE;
