/**
 * @file    bench_bignum_mul_bignum_mt.c
 * @brief   Многопоточный микробенчмарк для профилирования bignum_mul_bignum.
 ...
 *   выполняет свой набор вызовов bignum_mul_bignum, используя
 ...
 *   benchmarks/bench_bignum_mul_bignum_mt.c build/bignum_mul_bignum.o \
 *   -o bin/bench_bignum_mul_bignum_mt
 *
 * # Запуск perf
 * /usr/local/bin/perf record -F 9999 -o benchmarks/reports/report_bench_bignum_mul_bignum_mt -g -- \
 *   bin/bench_bignum_mul_bignum_mt
 */
...
typedef struct {
    unsigned thread_id;
    unsigned iters;
    const bignum_t* res; // Указатель на общий пул результата чисел
...
static void* thread_func(void *arg) {
    ...
    for (unsigned i = 0; i < t->iters; ++i) {
        ...
        bignum_t res_dst = t->res[data_idx];
